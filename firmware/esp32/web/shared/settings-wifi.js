window.CMWifiPage=(()=>{
  ...
  function networkStateText(value){const code=String(value||'UNKNOWN'),labels={AP_RECOVERY:'Только сервисная точка доступа',AP_STA_CONNECTED:'Точка доступа + подключение к роутеру',AP_STA_CONNECTING:'Подключение к роутеру',AP_NO_PROFILES:'Нет включённых профилей роутера',AP_STA_RETRY_WAIT:'Ожидание повторного подключения'};return labels[code]||('Неизвестно ('+code+')')}
  function networkResultText(value){const code=String(value||'UNKNOWN'),labels={NOT_STARTED:'ещё не запускалось',AP_START_FAILED:'не удалось запустить сервисную точку доступа',PROFILE_STORAGE_INVALID:'хранилище Wi‑Fi профилей недоступно или повреждено',CONNECTED:'подключено',PROFILE_TIMEOUT:'истекло время подключения к профилю',NO_ENABLED_PROFILES:'нет включённых профилей',CONNECTING:'выполняется подключение',IP_CONFIG_FAILED:'не удалось применить параметры IP'};return labels[code]||('Неизвестно ('+code+')')}
  ...
  async function load(){try{const [pr,sr]=await Promise.all([fetch('/api/network/profiles',{cache:'no-store'}),fetch('/api/system/network',{cache:'no-store'})]),p=await pr.json(),s=await sr.json();if(!pr.ok||!sr.ok)throw new Error('network_unavailable');items=p.items||[];render();$('runtime').innerHTML='<div class="row"><span>Состояние</span><b>'+esc(networkStateText(s.network_state))+'</b></div><div class="row"><span>AP</span><b>'+esc(s.ap_ssid)+' · '+esc(s.ap_ip)+'</b></div><div class="row"><span>STA</span><b>'+(s.sta_connected?esc(s.sta_ssid)+' · '+esc(s.sta_ip):(s.sta_connecting?'подключение…':'не подключена'))+'</b></div><div class="row"><span>Результат</span><b>'+esc(networkResultText(s.network_last_result))+'</b></div>'}catch(e){$('runtime').className='note bad';$('runtime').textContent='Сетевое состояние недоступно.';$('profiles').textContent='Профили недоступны.'}}
