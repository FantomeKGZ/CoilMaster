...
After `e5fc99e6...`, the operator explicitly confirmed on 2026-08-23 that all current Actions are GREEN. The operator then completed the physical ESP32 `/web` smoke and reported no errors; the verified reference bundle is hardware-smoke GREEN. A follow-up UX batch added the missing `📚 Справочник` entry to the shared desktop/mobile app shell, restored the effective anchor-link audit and made the SD bundle workflow rebuild for every `firmware/esp32/web/**` change. The operator explicitly confirmed all current Actions GREEN. The current follow-up preserves the exact opened generated page when switching desktop/mobile (`60ae0f54...` + `41717ae7...`) and improves reference search with order-independent multi-token matching, relevance ranking and URL query restoration (`fff7e338...` + `62667b66...` + `7c595062...`). Verify current CMP and Reference workflows, then deploy the refreshed artifact and smoke the shared-shell entry, same-page mode switch and representative multi-word searches.
