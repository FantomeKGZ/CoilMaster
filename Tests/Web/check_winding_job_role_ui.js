...
const pages = [
  ['desktop', 'firmware/esp32/web/desktop/winding-job.html'],
  ['mobile', 'firmware/esp32/web/mobile/winding-job.html']
];

for (const [variant, path] of pages) {
  const source = fs.readFileSync(path, 'utf8');
  function must(needle, label) {
    if (!source.includes(needle)) throw new Error(`${variant}: missing ${label}: ${needle}`);
  }
  ...
  must('RUN_COMPLETED', 'non-mutating completion wording');

  if (source.includes("fetch('/api/hardware/")) throw new Error(`${variant}: role UI must not introduce direct hardware control`);
  if (source.includes("repeatTarget').oninput")) throw new Error(`${variant}: linked repeat target must not remain operator-editable`);
}

console.log('Winding job role UI parity contract OK: desktop/mobile versioned WORKING/STARTING selection, fail-closed STARTING, readonly role program/repeat, existing linked POST path, and physical START/material safety are preserved.');
