...
{
    // The ordinary keypad B action is an absolute local operator exit, but it
    // must never become a silent cancellation channel for a job owned by ESP32.
    // Remote jobs are cancelled only through the explicit operator-abort/JOB_CANCEL
    // path in the Arduino entrypoint; final remote completion still waits for its
    // exact RUN_COMPLETED delivery acknowledgement.
    if (m_job.source == JobSource::Esp32Web && m_job.isValid())
        return false;

    // For local work B always means abandon the current local program and return
    // to HOME, including an in-progress/paused/manual/coil-boundary run. The
    // Arduino output layer observes EnterCoilCount in the same loop and removes
    // SSR authority; no automatic resume is created.
    resetToHome();
    return true;
}
