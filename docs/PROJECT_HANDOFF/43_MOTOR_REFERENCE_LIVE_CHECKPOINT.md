...
- Last confirmed `firmware/esp32/web/reference/motor-reference.json`: **1253 records**
- `reference_only=true`
- Count is merge-aware; enrichment supplements do not inflate `record_count`.
- `92c051d` adds 18 AOK2 5–7 frame base cards; generated count after that commit is still pending workflow regeneration at this exact handoff update.
...
- `92c051d`: workflow result pending at this exact update; do not call it green yet.
- Do not generalize these to all project CI unless separately confirmed.

## Exact next continuation point
1. Check `92c051d` specialized workflow and fetch the regenerated exact `record_count`.
2. **AOK2 rotor enrichment**: existing AOK2-41/42 stator cards are already in `AO2_41_42.source.json`; add rotor data only via `merge_only`, never duplicate them as new motors. For 5–7 frames, enrich the new `AOK2_5_7_PHASE_ROTOR_INDEX` cards only when section 8.2 row mapping is unambiguous.
3. **AK2 rotor/winding enrichment**: section 8.2 confirms 8–9 frame rotors use bare copper bars insulated with glass tape; add exact dimensions/slots/turn data only where row mapping is clean.
4. **6A 80/90**: keep searching for a textual/mirrored form of the image-based winding table. Add only confirmed rows; no OCR guessing.
5. **4A280/315/355**: separate winding-data section is known to exist, but current HTML extraction does not expose reliable columns yet. Add only when `N / conductor / pitch` map cleanly.
6. Continue AO2/A2 merge-only enrichment, VAO winding enrichment, then dedicated rectangular-LV source discovery.
