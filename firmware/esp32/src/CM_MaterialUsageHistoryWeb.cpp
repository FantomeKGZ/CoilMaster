...
void MaterialLedgerWeb::handleUsageHistory()
{
    ...
    const String repairIdSource = hasRepairId
        ? m_server.arg("repair_id") : String();
    if (hasRepairId && repairIdSource.length() > 0U &&
        !parseUnsignedValue(repairIdSource, 1UL, 0xFFFFFFFFUL, repairId))
    {
    ...
    bool hasMore = false;
    if (!m_ledger.appendUsageHistoryPageJson(
            response, repairId, materialId, cursor,
            static_cast<uint8_t>(parsedLimit), count, nextCursor, hasMore))
    {
        if (!m_ledger.ready())
        {
            m_server.send(503, "application/json; charset=utf-8",
