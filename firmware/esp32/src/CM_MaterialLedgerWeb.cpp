...
void MaterialLedgerWeb::handleUsage()
{
    ...
    String materialCurrency;bool materialFound=false;if(!m_ledger.loadActiveMaterialCurrency(materialId,materialCurrency,materialFound)){if(!m_ledger.ready())m_server.send(503,"application/json; charset=utf-8","{\"error\":\"materials_unavailable\"}");else m_server.send(500,"application/json; charset=utf-8","{\"error\":\"material_reference_read_failed\"}");return;}if(!materialFound){m_server.send(404,"application/json; charset=utf-8","{\"error\":\"material_not_found\"}");return;}
    if(materialCurrency!="KGS"){String error=F("{\"error\":\"unsupported_material_currency\",\"material_id\":");error+=materialId;error+=F(",\"material_currency\":\"");error+=materialCurrency;error+=F("\",\"supported_currency\":\"KGS\",\"currency_policy\":\"KGS_ONLY\",\"write_performed\":false}");m_server.send(409,"application/json; charset=utf-8",error);return;}
    ...
    if(!m_ledger.confirmUsage(usage,result)){if(!m_ledger.ready())m_server.send(503,"application/json; charset=utf-8","{\"error\":\"materials_unavailable\"}");else m_server.send(409,"application/json; charset=utf-8","{\"error\":\"usage_not_committed\",\"write_performed\":false}");return;}
